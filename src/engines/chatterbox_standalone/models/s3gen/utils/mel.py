...
# Use fallback for Python 3.14 compatibility (numba not available)
try:
    from librosa.filters import mel as librosa_mel_fn
except (ImportError, ModuleNotFoundError):
    # Fallback to torchaudio-based mel filters for Python 3.14
    import sys
    import os
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..', '..'))
    from utils.audio.librosa_fallback import safe_mel_filters
    def librosa_mel_fn(sr, n_fft, n_mels=80, fmin=0.0, fmax=None):
        return safe_mel_filters(sr, n_fft, n_mels, fmin, fmax)
import torch
import numpy as np
