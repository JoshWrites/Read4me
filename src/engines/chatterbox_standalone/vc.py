...
# Import from local utils (standalone version)
import sys
import os
# Add src directory to path to find utils
_current_dir = os.path.dirname(os.path.abspath(__file__))
_src_dir = os.path.dirname(os.path.dirname(_current_dir))  # Go up to src/
...
class ChatterboxVC:
    ...
    def from_local(cls, ckpt_dir, device) -> 'ChatterboxVC':
        ...
        # Determine proper map_location for device (handle CUDA models on non-CUDA devices)
        def get_map_location(target_device):
            """Get proper map_location for loading CUDA-saved models on any device"""
            if target_device == "cuda" and torch.cuda.is_available():
                return "cuda"
            elif target_device == "mps" or (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()):
                return "mps"
            else:
                return "cpu"
    ...
    def generate(
        ...
        Args:
            audio: Can be file path (str) or audio tensor (torch.Tensor or np.ndarray with shape [channels, samples] or [samples])
            target_voice_path: Optional target voice path (if ref_dict not set)
        """
        ...
        with torch.inference_mode():
            # Handle tensor input directly (eliminates temp file I/O)
            if isinstance(audio, torch.Tensor):
                # Tensor input - process directly
                audio_tensor = audio
                if audio_tensor.dim() == 1:
                    # [samples] -> [1, samples]
                    audio_tensor = audio_tensor.unsqueeze(0)
                elif audio_tensor.dim() == 2:
                    # [channels, samples] -> ensure mono by taking first channel
                    if audio_tensor.shape[0] > 1:
                        audio_tensor = audio_tensor[0:1]  # Take first channel
                elif audio_tensor.dim() == 3:
                    # [batch, channels, samples] -> [1, samples]
                    audio_tensor = audio_tensor.squeeze(0)
                    if audio_tensor.shape[0] > 1:
                        audio_tensor = audio_tensor[0:1]
                
                # Resample to S3_SR if needed (assuming input is at some sample rate)
                # For now, assume input is already at correct rate or close enough
                # TODO: Add proper resampling if needed
                audio_16 = audio_tensor.float().to(self.device)
            elif isinstance(audio, (list, tuple)) and len(audio) == 2:
                # Tuple of (waveform, sample_rate) - extract waveform and resample if needed
                audio_tensor = audio[0]
                input_sr = audio[1] if len(audio) > 1 else S3_SR
                ...
                if isinstance(audio_tensor, torch.Tensor):
                    if audio_tensor.dim() == 1:
                        ...
                        if audio_tensor.shape[0] > 1:
                            audio_tensor = audio_tensor[0:1]  # Take first channel for mono
                    elif audio_tensor.dim() == 3:
                        audio_tensor = audio_tensor.squeeze(0)
                        if audio_tensor.shape[0] > 1:
                            audio_tensor = audio_tensor[0:1]
                    
                    # Resample to S3_SR if needed
                    if input_sr != S3_SR:
                        import torchaudio
                        resampler = torchaudio.transforms.Resample(input_sr, S3_SR)
                ...
                # File path - load using safe_load (backward compatibility)
                audio_16_wav, _ = safe_load(audio, sr=S3_SR, mono=True)
                audio_16 = torch.from_numpy(audio_16_wav).float().to(self.device)[None, ]
